...
首批三张背景图片、骑兵行军序列帧、地域城池与关隘图片、全部 112 名武将全身肖像使用 Codex 内置生图能力生成，生成日期为 2026-07-13。2026-07-15 新增松柏与桃花枝两张大型面板角花，同样由 Codex 内置生图能力生成。它们是项目概念和首版界面资产，不含正式 UI、文字、数值和商标。
...
- 松柏角花：左上 L 形构图，老松树干与针叶沿上沿和左沿自然伸展。
- 桃花角花：右下 L 形构图，老桃枝与疏朗花朵沿右沿和下沿生长。
- 源文件：`generated/ui-corner-pine-source-v1.png`、`generated/ui-corner-peach-source-v1.png`。
- 运行文件：`runtime/ui/ink-pine-corner-v1.png`、`runtime/ui/ink-peach-corner-v1.png`，RGBA 透明背景。
- 色彩限制：所有可见像素严格为黑白灰，RGB 三通道相等；禁止粉色花瓣、绿色松针、棕色树干和其他彩色晕染。
- 程序接入：角花只作为大型宣纸面板的低透明度装饰，鼠标穿透，不承载文字、按钮、数值或状态。

提示词摘要：东亚传统水墨画，纯黑墨、灰墨和纸白，松柏左上角或桃花枝右下角，克制留白，适合历史策略游戏宣纸面板；纯洋红抠图背景，无文字、印章、UI、边框、水印和任何彩色颜料。
